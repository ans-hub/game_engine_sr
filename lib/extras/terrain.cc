...
// The same as above but returns normal of ground at the position 
...
Vector Terrain::FindGroundNormal(const Vector& pos) const
{
  ...
  // If all vertices is inside mesh, then find normal by billinear interpolation

  if (lt < vxs_.size() && lb < vxs_.size() && rt < vxs_.size() && rb < vxs_.size())
  {
    float dx = col_float - col_floor;
    float dy = row_float - row_floor;
    Vector normal {};
    normal += vxs_[lt].normal_ * (1.0f-dx) * (1.0f-dy);
    normal += vxs_[rt].normal_ * (dx) * (1.0f-dy);
    normal += vxs_[lb].normal_ * (1.0f-dx) * (dy);
    normal += vxs_[rb].normal_ * (dx) * (dy);
    return normal;
  }
  else
    return {};
