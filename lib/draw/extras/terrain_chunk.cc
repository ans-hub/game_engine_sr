...
// Creates GlObject that is chunk of terrain

TerrainChunk::TerrainChunk(const char* map_fname, const char* tex_fname, int div_factor)
  : GlObject()
  , hm_w_{}
  , hm_h_{}
  , tx_w_{}
  ...
  , det_faces_{}
{
  ...
  constexpr int kStartDetalization {1};
  faces_ = ComputeFaces(kStartDetalization);
  det_faces_.push_back(faces_);
}

void TerrainChunk::ComputeVertices(const Bitmap& map, int div_factor)
{
...
V_Face TerrainChunk::ComputeFaces(int det)
{
  std::vector<Face> res {};
...
void TerrainChunk::SetDetalization(
...
void TerrainChunk::UseDetalization(const Vector& vrp)
{
